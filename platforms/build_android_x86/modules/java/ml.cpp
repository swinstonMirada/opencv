...
//
//  float CvNormalBayesClassifier::predict(Mat samples, Mat* results = 0, Mat* results_prob = 0)
//

JNIEXPORT jfloat JNICALL Java_org_opencv_ml_CvNormalBayesClassifier_predict_10 (JNIEnv*, jclass, jlong, jlong, jlong, jlong);

JNIEXPORT jfloat JNICALL Java_org_opencv_ml_CvNormalBayesClassifier_predict_10
  (JNIEnv* env, jclass , jlong self, jlong samples_nativeObj, jlong results_nativeObj, jlong results_prob_nativeObj)
{
    static const char method_name[] = "ml::predict_10()";
    try {
        LOGD("%s", method_name);
        ...
        Mat& samples = *((Mat*)samples_nativeObj);
        Mat& results = *((Mat*)results_nativeObj);
        Mat& results_prob = *((Mat*)results_prob_nativeObj);
        float _retval_ = me->predict( samples, &results, &results_prob );
        return _retval_;
    } catch(const std::exception &e) {
        throwJavaException(env, &e, method_name);
    } catch (...) {
